  ...
  function render() {
    ...
    if (summary) {
      summary.innerHTML =
        ...
        "In this toy cloud the Cech complex stays inside the Vietoris-Rips complex, so the relaxation adds simplices earlier.";
    }
  }
  ...
  function render() {
    ...
    if (summary) {
      ...
      summary.innerHTML =
        ...
        "Connected components merge first, then a cycle appears, and finally a 2-simplex turns that cycle into a boundary.";
    }
  }
  ...
  function render() {
    ...
    if (summary) {
      summary.innerHTML =
        ...
        "Rotating first and then averaging gives the same result as averaging first and rotating afterward: that is the GENEO equivariance principle.";
    }
  }
...
export function enhanceTdaNotesPage(root = document) {
  const page = enhanceMathNotebookPage(root, {
    ...
    bodyClass: "tda-notes-page",
    tocOptions: { jumpLabel: "Jump to lecture section" }
  });
